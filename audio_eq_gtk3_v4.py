...
class AudioNormalizer:
    ...
    def __init__(self):
        ...
        self.smoothing_factor = 0.1  # Smooth factor
        self.target_gain = 0.0
        self.current_gain = 0.0
        self.gain_history = deque(maxlen=20)  # Store gain history
    ...
    def calculate_normalization_gain(self):
        ...
        if self.current_lufs > -70:
            ...
            target_gain = min(lufs_gain, headroom)
            
            # Limit gain change rate
            if len(self.gain_history) > 0:
                last_gain = self.gain_history[-1]
                max_change = 1.0  # Max change per update
                target_gain = max(last_gain - max_change, 
                                min(last_gain + max_change, target_gain))
            
            # Smooth transition
            self.target_gain = target_gain
            self.current_gain = self.current_gain * (1 - self.smoothing_factor) + target_gain * self.smoothing_factor
...
class AudioAnalyzer:
    ...
    def generate_eq_curve(self, imperfections, weights=None, num_bands=10):
        ...
        if weights is None:
            weights = {band: 0.0 for band in self.freq_bands}  # Default to 0dB
        
        for band_name, issues in imperfections.items():
            for issue in issues:
                freq = issue['frequency']
                correction = issue['correction']
                # Apply weight as multiplier (convert dB to linear, multiply, convert back)
                weight_db = weights.get(band_name, 0.0)
                weight_linear = 10 ** (weight_db / 20)
                corrected_gain = correction * weight_linear
                
                closest_band = np.argmin(np.abs(eq_freqs - freq))
                weight = 1.0 / (1.0 + np.abs(eq_freqs[closest_band] - freq) / 1000)
                
                eq_gains[closest_band] += corrected_gain * weight * issue['severity']
                
                # Rozprzestrzeń na sąsiednie pasma
                if closest_band > 0:
                    eq_gains[closest_band - 1] += corrected_gain * weight * 0.3
...
class BandWeightWidget(Gtk.Box):
    ...
    def __init__(self, analyzer):
        ...
        for band_name in self.analyzer.freq_bands.keys():
            ...
            slider.set_range(-6, 6)  # Range from -6dB to +6dB
            slider.set_value(0)
            slider.set_digits(1)  # Show one decimal place
            slider.set_draw_value(True)
            # Set step size to 0.1dB
            slider.set_increments(0.1, 0.5)  # Small step 0.1, big step 0.5
            slider.connect("value-changed", self.on_weight_changed, band_name)
            hbox.pack_start(slider, True, True, 0)
            
            # Add current value display
            value_label = Gtk.Label(label="0.0 dB")
    ...
    def on_weight_changed(self, slider, band_name):
        ...
        value_label = self.sliders[band_name][1]
        value_label.set_text(f"{value:.1f} dB")
        print(f"Zmieniono wagę dla pasma {band_name} na {value:.1f} dB")

    def get_weights(self):
        weights = {}
        for band_name, (slider, _) in self.sliders.items():
...
class AudioAnalyzerApp(Gtk.Window):
    ...
    def __init__(self):
        ...
        self.dynamic_eq_mode = False  # New: Dynamic EQ mode flag
        self.dimm_enabled = False
        self.bass_level = -70.0
        self.treble_level = -70.0
        self.dimm_amount = 0.0
        self.dimm_smoothing = 0.2  # Smooth factor
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        # Relaxing Dimm Audio button
        self.dimm_button = Gtk.ToggleButton(label="Relaxing Dimm OFF")
        self.dimm_button.connect("toggled", self.on_dimm_toggle)
        toolbar.pack_start(self.dimm_button, False, False, 0)
        ...
        # Lewa strona: widżety analizy
        left_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=5)
        main_hbox.pack_start(left_box, True, True, 0)

        # Spektrum i EQ
        spectrum_frame = Gtk.Frame()
        spectrum_frame.set_label("Spektrum i EQ")
        left_box.pack_start(spectrum_frame, True, True, 0)
        
        self.spectrum_widget = SpectrumWidget()
        self.spectrum_widget.analyzer = self.analyzer
        spectrum_frame.add(self.spectrum_widget)

        # Analiza pasm
        band_frame = Gtk.Frame()
        band_frame.set_label("Przebiegi pasm")
        left_box.pack_start(band_frame, True, True, 0)
        
        self.band_analysis_widget = BandAnalysisWidget(self.analyzer)
        band_frame.add(self.band_analysis_widget)

        # Środek: wagi i raport
        middle_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=5)
        main_hbox.pack_start(middle_box, False, False, 0)

        # Wagi pasm
        weight_frame = Gtk.Frame()
        weight_frame.set_label("Wagi pasm")
        middle_box.pack_start(weight_frame, False, False, 0)
        
        self.band_weight_widget = BandWeightWidget(self.analyzer)
        weight_frame.add(self.band_weight_widget)

        # Raport
        report_frame = Gtk.Frame()
        report_frame.set_label("Raport")
        middle_box.pack_start(report_frame, True, True, 0)
        ...
        report_frame.add(report_scroll)
        ...
        # Dimm Audio info
        dimm_frame = Gtk.Frame()
        dimm_frame.set_label("Info Dimm Audio")
        right_box.pack_start(dimm_frame, False, False, 0)
        
        dimm_info_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=5)
        dimm_info_box.set_margin_start(10)
        dimm_info_box.set_margin_end(10)
        dimm_info_box.set_margin_top(10)
        dimm_info_box.set_margin_bottom(10)
        dimm_frame.add(dimm_info_box)
        
        self.bass_level_label = Gtk.Label(label="Bas: -- dB")
        dimm_info_box.pack_start(self.bass_level_label, False, False, 0)
        
        self.treble_level_label = Gtk.Label(label="Soprany: -- dB")
        dimm_info_box.pack_start(self.treble_level_label, False, False, 0)
        
        self.dimm_amount_label = Gtk.Label(label="Wyciszenie: 0.0 dB")
        dimm_info_box.pack_start(self.dimm_amount_label, False, False, 0)
    ...
    def apply_normalization_gain(self):
        ...
        if self.normalize_enabled and self.is_playing:
            ...
            gain_linear = max(0.5, min(2.0, gain_linear))
            self.volume.set_property("volume", gain_linear)
            return True  # Kontynuuj timeout
        return False  # Zatrzymaj timeout

    def on_dimm_toggle(self, button):
        """Toggle relaxing dimm audio feature"""
        self.dimm_enabled = button.get_active()
        
        if self.dimm_enabled:
            button.set_label("Relaxing Dimm ON")
            self.status_label.set_text("Relaxing Dimm włączony")
            # Start dynamic dimm update
            GLib.timeout_add(100, self.update_dimm_audio)
        else:
            button.set_label("Relaxing Dimm OFF")
            self.status_label.set_text("Relaxing Dimm wyłączony")
            # Reset dimm effect
            self.reset_dimm_effect()
    
    def update_dimm_audio(self):
        """Dynamically update relaxing dimm audio effect"""
        if not self.dimm_enabled or not self.is_playing:
            return False
        
        # Get low and high frequency levels
        if len(self.analyzer.band_history['low']) > 0 and len(self.analyzer.band_history['high']) > 0:
            low_metrics = list(self.analyzer.band_history['low'])[-1]
            high_metrics = list(self.analyzer.band_history['high'])[-1]
            
            self.bass_level = low_metrics['mean']
            self.treble_level = high_metrics['mean']
            
            # Calculate dimm amount: higher bass means more dimming of treble and mid
            # But limit maximum dimming
            bass_normalized = (self.bass_level + 70) / 70  # Normalize to 0-1
            target_dimm = bass_normalized * 6.0  # Max 6dB dimming
            
            # Smooth transition
            self.dimm_amount = self.dimm_amount * (1 - self.dimm_smoothing) + target_dimm * self.dimm_smoothing
            
            # Apply to high frequency and mid frequency sliders
            high_freq_indices = [7, 8, 9]  # 4kHz, 8kHz, 16kHz
            mid_freq_indices = [4, 5, 6]   # 500Hz, 1kHz, 2kHz
            
            for i in high_freq_indices:
                if i < len(self.eq_sliders):
                    slider, db_label = self.eq_sliders[i]
                    current_value = slider.get_value()
                    # Apply dimming, but not below original value-6dB
                    new_value = max(current_value - self.dimm_amount, current_value - 6.0)
                    slider.set_value(new_value)
                    db_label.set_text(f"{new_value:.1f} dB")
                    self.equalizer.set_property(f"band{i}", new_value)
            
            for i in mid_freq_indices:
                if i < len(self.eq_sliders):
                    slider, db_label = self.eq_sliders[i]
                    current_value = slider.get_value()
                    # Apply less dimming, but not below original value-3dB
                    new_value = max(current_value - self.dimm_amount * 0.5, current_value - 3.0)
                    slider.set_value(new_value)
                    db_label.set_text(f"{new_value:.1f} dB")
                    self.equalizer.set_property(f"band{i}", new_value)
            
            # Update UI labels
            self.bass_level_label.set_text(f"Bas: {self.bass_level:.1f} dB")
            self.treble_level_label.set_text(f"Soprany: {self.treble_level:.1f} dB")
            self.dimm_amount_label.set_text(f"Wyciszenie: {self.dimm_amount:.1f} dB")
        
        return True  # Continue updating
    
    def reset_dimm_effect(self):
        """Reset dimm effect"""
        self.dimm_amount = 0.0
        self.bass_level_label.set_text("Bas: -- dB")
        self.treble_level_label.set_text("Soprany: -- dB")
        self.dimm_amount_label.set_text("Wyciszenie: 0.0 dB")
    ...
    def on_apply_eq(self, widget):
        """Apply auto EQ curve, toggle static/dynamic mode"""
        if not self.has_audio_data:
            self.status_label.set_text("Brak danych - rozpocznij odtwarzanie")
            return
        
        # Toggle dynamic EQ mode
        self.dynamic_eq_mode = not self.dynamic_eq_mode
        
        if self.dynamic_eq_mode:
            self.apply_eq_button.set_label("Zatrzymaj Auto-EQ")
            self.status_label.set_text("Tryb dynamiczny EQ aktywny")
            # Start dynamic EQ update
            GLib.timeout_add(100, self.update_dynamic_eq)
        else:
            self.apply_eq_button.set_label("Zastosuj Auto-EQ")
            self.status_label.set_text("Tryb statyczny EQ aktywny")
            # Apply static EQ once
            self.apply_static_eq()
    
    def apply_static_eq(self):
        """Apply static EQ curve"""
        if len(self.analyzer.band_history['low']) == 0:
            self.status_label.set_text("Brak wystarczających danych")
            return
        
        # Create band_analysis from history
        band_analysis = {}
        for band_name in self.analyzer.freq_bands.keys():
    ...
    def update_dynamic_eq(self):
        """Dynamically update EQ curve"""
        if not self.dynamic_eq_mode or not self.is_playing:
            return False
        
        if len(self.analyzer.band_history['low']) == 0:
            return True  # Continue trying
        
        # Create band_analysis from latest data
        band_analysis = {}
        for band_name in self.analyzer.freq_bands.keys():
            if len(self.analyzer.band_history[band_name]) > 0:
                recent = list(self.analyzer.band_history[band_name])[-1]
        ...
        # Smooth update EQ sliders
        for i, (slider, db_label) in enumerate(self.eq_sliders):
            current_gain = slider.get_value()
            target_gain = eq_gains[i]
            # Limit change rate
            max_change = 0.5  # Max change per update
            new_gain = max(current_gain - max_change, 
                          min(current_gain + max_change, target_gain))
            slider.set_value(new_gain)
            db_label.set_text(f"{new_gain:.1f} dB")
            self.equalizer.set_property(f"band{i}", new_gain)
